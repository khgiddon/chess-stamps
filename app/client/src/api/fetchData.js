...
export const fetchData = async (username, timeframe, setData, setProgress, setGamesExpected) => {
...
const handleProgress = (progressData) => {
  ...
  try {
        const response = await fetch(`http://127.0.0.1:5000/openings?username=${username}&timeframe=${timeframe}`);
        if (!response.ok) {
